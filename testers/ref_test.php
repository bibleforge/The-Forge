...
require_once '../data/ref_array_' . LANG . '.php';
...
for (j in refs) {
    for (i in refs[j]) {
        ...
        book = BF.langs.<?php echo LANG ?>.determine_reference(refs[j][i]);
        if (book !== false) {
        ...
        document.write("<tr><td>" + refs[j][i] + "</td><td class=" + (books_re_worked ? "worked" : "notworked") + ">" + books_re_worked + "</td><td class=" + (book_arr_re_worked ? "worked" : "notworked") + ">" + book_arr_re_worked + " " + BF.langs.<?php echo LANG ?>.books_short[book] + "</td><td class=" + (re_other_worked ? "worked" : "notworked") + ">" + re_other_worked + "</td></tr>")
